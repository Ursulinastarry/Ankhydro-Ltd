...
async function refreshAccessToken() {
    const response = await axios.post(`${getZohoAccountsBaseUrl()}/oauth/v2/token`, null, {
        params: {
...
export async function sendZohoEmail({ to, subject, text, html }) {
    ...
    try {
        for (let attempt = 1; attempt <= 2; attempt += 1) {
            try {
                ...
                await axios.post(`${getZohoMailBaseUrl()}/api/accounts/${accountId}/messages`, {
...
export async function sendNotificationEmail(to, title, message, data) {
...
export async function createAndSendNotification(payload) {
    ...
    if (audience === "ADMIN") {
        const { rows: admins } = await pool.query(`SELECT id, email FROM users WHERE is_admin = TRUE`);
        const created = [];
        for (const admin of admins) {
            const id = `notif-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
            const { rows } = await pool.query(`INSERT INTO notifications (id, user_id, type, title, message, data, is_read)
