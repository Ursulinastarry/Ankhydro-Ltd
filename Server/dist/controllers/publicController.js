...
function getStatsMap(row) {
    if (!row)
        return { boreholes: 0, solar: 0, clients: 0, counties: 0 };
    return {
...
export async function submitContact(req, res) {
    ...
    try {
        const result = await queryOne('INSERT INTO contacts (name, email, phone, service, message) VALUES ($1, $2, $3, $4, $5) RETURNING id, created_at', [name, email, phone || null, service || null, message]);
...
export async function submitQuote(req, res) {
    ...
    try {
        const result = await queryOne('INSERT INTO quotes (name, email, phone, location, service, package, description, contact_method) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id, created_at', [name, email, phone || null, location, service || null, pkg || null, description, contact_method || null]);
...
export async function getSiteData(_req, res) {
    try {
        ...
        res.json({
            ...
            packages: packages.map((p) => ({ ...p, order: p.display_order, priceLabel: p.price_label, service_id: p.service_id })),
...
export async function publishSiteData(_req, res) {
    try {
        const [settingsRow, statsRow, services, packages, testimonials, team, faq, projects, blog] = await Promise.all([
            queryOne('SELECT * FROM site_settings ORDER BY updated_at DESC LIMIT 1'),
            queryOne('SELECT * FROM site_stats ORDER BY updated_at DESC LIMIT 1'),
...
export async function uploadImage(req, res) {
    try {
        ...
        await queryOne('INSERT INTO file_uploads (filename, section, item_id, url, mime_type, size) VALUES ($1,$2,$3,$4,$5,$6) RETURNING id', [filename, section, itemId, fileUrl, image.mimetype, image.size]);
